...
from .serializers import StockStatusSerializer , StockStatusUpdateLogSerializer , StockStatusPatchSerializer , StockStatusChangeSessionSerializer , StockStatusFieldLogSerializer , DebitEntrySerializer , ContractualHistorySerializer , DashboardSnapshotSerializer , DashboardOrderSerializer
...
class StockDashboard(APIView):
    ...
    def get(self, request):
        ...
        # Display order is configured in the DashboardOrder table (sorted by
        # order_number). item_code values here are tank_item_code strings, which
        # is exactly what DashboardOrder.item_code stores (to_field='tank_item_code').
        item_display_order = {}
        item_dashboard_id = {}
        for row in DashboardOrder.objects.values('id', 'item_code_id', 'order_number'):
            item_display_order[row['item_code_id']] = row['order_number']
            item_dashboard_id[row['item_code_id']] = row['id']

        def item_sort_key(item_code):
            if item_code in item_display_order:
                return (0, item_display_order[item_code])
        ...
        for item_code in ordered_items:
            ...
            items.append({
                ...
                'dashboard_id': item_dashboard_id.get(item_code),  # DashboardOrder id, for updating this item's order
...
class DashboardOrderCreateView(generics.CreateAPIView):
    queryset = DashboardOrder.objects.all()
    serializer_class = DashboardOrderSerializer
    def get_permissions(self):
        return [IsAuthenticated() , HasAppPermission('stock.add_dashboardorder')]


class DashboardOrderUpdateView(generics.ListAPIView):
    queryset = DashboardOrder.objects.all().order_by('order_number')
    serializer_class = DashboardOrderSerializer
    def get_permissions(self):
        return [IsAuthenticated() , HasAppPermission('stock.view_dashboardorder')]


class DashboardOrderDetailView(generics.RetrieveUpdateDestroyAPIView):
    queryset = DashboardOrder.objects.all()
    serializer_class = DashboardOrderSerializer
    lookup_field = 'id'
    ...
    def get_permissions(self):
        if self.request.method == 'DELETE':
            return [IsAuthenticated() , HasAppPermission('stock.delete_dashboardorder')]
        if self.request.method in ['PUT' , 'PATCH']:
