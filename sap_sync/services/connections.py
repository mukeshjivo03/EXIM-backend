...
"""
    ...
    def get_total_finsihed_qty(self):
        return """
        SELECT  SUM(LITER) AS 'Finished Qty'
            FROM   
            (
                SELECT * FROM OPENQUERY(HANADB112, '
                    SELECT ''New'' SAP, T0."Warehouse", ''CB'', ''Closing'',
    ...
    def balance_sheet_insights():
        ...
        inner_query_desc = (
            "WITH \"VendorLatestTransaction\" AS ("
            "SELECT a.\"CardCode\", a.\"CardName\", CAST(a.\"Balance\" AS DECIMAL(18,2)) AS \"Balance\", "
            "b.\"RefDate\" AS \"Last Transaction Date\", CAST(b.\"Debit\" AS DECIMAL(18,2)) AS \"Last Transanction Amount\", "
            "ROW_NUMBER() OVER (PARTITION BY a.\"CardCode\" ORDER BY b.\"RefDate\" DESC) AS \"RowNum\" "
            "FROM \"JIVO_OIL_HANADB\".\"OCRD\" a "
            "INNER JOIN \"JIVO_OIL_HANADB\".\"JDT1\" b ON a.\"CardCode\" = b.\"ShortName\" "
            f"WHERE b.\"RefDate\" >= ''2024-10-01'' AND b.\"Debit\" > 0 AND a.\"CardCode\" IN ({card_codes})) "
            "SELECT \"CardCode\", \"CardName\", \"Balance\", \"Last Transaction Date\", \"Last Transanction Amount\" "
            "FROM \"VendorLatestTransaction\" WHERE \"RowNum\" = 1 AND \"Balance\" <> 0 ORDER BY \"Balance\" DESC"
        )
        ...
        inner_query_asc = (
            ...
            f"WHERE b.\"RefDate\" >= ''2024-10-01'' AND b.\"Debit\" > 0 AND a.\"CardCode\" IN ({card_codes})) "
            "SELECT \"CardCode\", \"CardName\", \"Balance\", \"Last Transaction Date\", \"Last Transanction Amount\" "
            "FROM \"VendorLatestTransaction\" WHERE \"RowNum\" = 1 AND \"Balance\" <> 0 ORDER BY \"Balance\""
        )
    
        return (
            f"SELECT TOP (5) * FROM (SELECT * FROM OPENQUERY(HANADB112, '{inner_query_desc}')) AS Result "
            "UNION ALL "
    ...
    @staticmethod
    def get_balance_in_range(startDate  , endDate):
        return f"""
        SELECT SUM(Balance) AS "Balance" FROM    
            (SELECT * FROM OPENQUERY(HANADB112, '
                 ...
                 T0."CardName",
                 T5."SlpName",
                 CAST(T0."Balance" AS DECIMAL(18,2)) AS "OutstandingAmount",
                 CAST(
                      COALESCE((
                        SELECT SUM(I."DocTotal")
                        FROM "JIVO_BEVERAGES_HANADB"."OINV" I
                      ...
                      COALESCE((
                        ...
                        FROM "JIVO_BEVERAGES_HANADB"."ORCT" R
                        WHERE R."CardCode" = T0."CardCode"
                        AND R."TaxDate" BETWEEN ''{startDate}'' AND ''{endDate}''
                        AND R."Canceled" = ''N''
                        ), 0)
    ...
    def get_aging_customer():
        ...
        SELECT * FROM OPENQUERY(
            ...
            FROM "JIVO_BEVERAGES_HANADB"."OINV" T0
            INNER JOIN "JIVO_BEVERAGES_HANADB"."OCRD" T3 ON T0."CardCode" = T3."CardCode"
            INNER JOIN "JIVO_BEVERAGES_HANADB"."OSLP" T4 ON T3."SlpCode" = T4."SlpCode"
            WHERE
                T0."DocType" = ''I''
                AND T0."CANCELED" = ''N''
